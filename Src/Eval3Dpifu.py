...
from Dataset.PifuIFS import PifuIFS
...
if __name__ == "__main__":
    ...
    results_path = "../ifs_results_path/0714_pifu"

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    # set cuda
    net = IFSNet(None).to(device=device)

    load_model(net, "../checkpoints/[25-07-14-19-04-47] pifu_0714_1 ifs refactoring version refactoring version-e50.pth")

    dataset = PifuIFS(
        path=path, split="test", roi=roi, nov=8, yaw_list=[0, 6, 12, 18, 24, 30, 36, 42]
    )
