"""
created by Allan Yu, 14-10-03
"""

from copy import deepcopy
from LocalOnly.PathConfig import path_config


ifs_config = {
    "device": 'cuda:1',
    "epochs": 200,  #20
    "reload": False,
    "chkpt": None,
    "chkpt_milestones": [50, 100, 199], #[3, 5, 9, 14, 19], #[10,15,19]
    "sch_epoch": [50, 100, 199],#[3, 5, 9, 14, 19], #[10,15,19]
    "tr_batch_size": 1, #3
    "batchs_in_step": 16, #8
    "num_workers": 8,
    "lr": 1e-4,
    "lr_weight_decay": 0,
    "sch_gamma": 0.1,
    "apply_weight_init": True,
    "seed": 17,
    "topic": '0302_ifs_wo_localsample_3dcnn'
}


def create_ifs_dataset(tm, is_train=False):
    #from Dataset.PathedIFS import get_dataloader
    from Dataset.PifuIFS import get_dataloader
    from Model.IFSNet import IFSNet

    return get_dataloader(path=path_config["path_of_ifs"], is_train=is_train, roi=768, yaw_list=[0,6,12,18,24,30,36,42], dataset_type='A')


def create_model_ifs(tm):
    from Dataset.PathedIFS import ifs_pack
    from Model.IFSNet import IFSNet

    return IFSNet(tm, pack=ifs_pack, rov=64, fusion='wheel',decoder_type='mlp', **tm.args)


def task():
    config = deepcopy(ifs_config)
    config.update({"abstract": "0302_ifs_wo_localsample_3dcnn", "best_performance": 0.01})
    return {
        "name": "ifs",
        "create_dataloader": create_ifs_dataset,
        "create_model": create_model_ifs,
        "args": config,
    }
